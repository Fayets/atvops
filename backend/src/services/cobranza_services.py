...
class CobranzaServices:
    ...
    def _fetch_cobranza_mes(self, month: str) -> dict:
        ...
        url = f"{self._clients_base()}/api/agent/cobranza-mes?month={month}"
    ...
    def resumen(self, month: str | None = None) -> dict:
        ...
        for c in raw.get("cuotas") or []:
            ...
            if estado == "vencida" and vence:
                dias_atraso = max(0, (hoy - date.fromisoformat(vence[:10])).days)
            cuotas.append({
                "id": str(c.get("id")),
                "cliente": c.get("cliente") or "—",
                "clienteId": c.get("cliente_id"),
                "plan": c.get("plan") or "—",
                "montoUsd": float(c.get("monto_usd") or 0),
                "venceAt": vence,
                "estado": estado,
                "pagadaAt": c.get("pagada_at"),
                "tipo": c.get("tipo"),
        ...
        por_vencer = [
            ...
            and c["venceAt"]
            and hoy.isoformat() < c["venceAt"][:10] <= en7.isoformat()
        ]
        ...
        esperado_hoy = sum(
            ...
            if c["venceAt"] and c["venceAt"][:10] <= hoy.isoformat()
        )
